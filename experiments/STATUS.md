...
R038 人工 Webots 复跑显示：绝大多数弯不再剐蹭，轻蹭也能自己擦出；残留问题是弯中转弯半径仍偏小，车身会落到中间白线内侧，至少一处碰栏/轻蹭。
...
- 时间：`t=0.03→330.14s`
- telemetry：未记录 collision/checkpoint 等事件，无近停，末帧 `status=normal`；这不等于没碰栏，R038 人工确认仍有轻蹭
- 控制日志：全程 `lost=0`，`mean|lat|=0.069`，低命令速度 `<0.3` 约 1%
- 人工观察：多数弯已经不剐蹭，但仍有碰栏/轻蹭，只是不会卡死
...
- 不要只看 `lost=0` 或 telemetry 无 collision。R038 已经无 lost、telemetry 无事件，但人眼仍看到碰栏/轻蹭和半径偏小。
