...
- 最近全套 pytest 已通过（149 tests）；两个 submission 已重建，本地/官方 validator 都通过且不再触发 W014。当前官方 validator 性能：`no_other_cars` p95≈0.104ms，`with_other_cars` p95≈5.417ms。真实复测：R069/R071 单车优化后/优化前同环境成绩完全一致（`total_time=254.495s`），R070 6车 telemetry 显示 `ours` 第2完赛且 `best_lap=256.864s`，与 R065 持平。提交文件不含调试 I/O。
...
6. ~~**W014 性能软警告**~~ ✅ 已处理：对无色差占位/合成帧走快速失败路径，真实 Webots 帧不走该路径；24 组真实左右帧对比优化前后 `control()` 输出完全一致。R069/R071 单车同环境对照成绩一致，R070 6车 telemetry 与 R065 持平，未见策略性能下降。若继续提真实运行性能，再 profile `perception.py/opponent.py` 的 HSV/Lab mask 和 opponent 检测。
