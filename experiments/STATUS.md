...
最后更新：2026-06-12（白线感知 **Phase 2.2 候选已实现并完成 AI 短测 R029/R030/R031/R032/R033/R034/R035/R036**：R028 通过第一个左弯窗口，但 `t≈32→35` 仍有短暂白线掉线和低速硬左。R029 查明原因是弯中稀疏虚线横向跳变略超门槛、单侧相机短时凑不齐双目条件。当前候选只在 complex 红色环境启用低置信单目白线兜底，放宽曲线虚线 `max_center_jump_ratio 0.24→0.32`、`min_y_span 70→60`，并把 offset-heading 冲突的回中优先阈值 `0.30→0.18`。AI complex 长跑 R035 到 `t≈399.7s`，通过第一个左弯、旧 R024/R025 后段、R018 风险窗、旧 R024 `x≈-42,y≈124`、旧起点前 `x≈-10,y≈-27` / `x≈28,y≈-28`，并进入下一轮早段；无事件、无近停，overlay 未见栏杆/车辆误锁。R035 白线居中审计显示 `t>5s` 且 `line_conf>=0.6` 时 `|line_offset|` 中位数 `0.043`、p90 `0.288`，旧起点前和下一轮早段中位数分别为 `0.026` / `0.004`。R036 又用实际 `submissions/final/team_controller.py` 复跑到 `t≈360.3s`，关键旧窗口同样 normal、无近停。**这是目前最强 AI 证据，但走线/驾驶改动仍需人眼终判视觉上是否沿中间白色虚线；R026/R027 第一个左弯 case 仍 open，未合 main。**
...
**R032 R018 风险窗口短测**（2026-06-12，已完成，不跑完整场）：
- 配置：`bash scripts/webots_run.sh complex --frames 6`，AI 在 `t≈351.68s` 主动停止，从头覆盖 R018 的 `280→330s` 风险窗口。
...
**R035 长跑**（2026-06-12，已完成，不跑完整场）：
- 配置：`bash scripts/webots_run.sh complex --frames 12`，AI 跑过 R034 之后的后段并进入下一轮早段，在 `t≈399.74s` 主动停止。
- 结果：telemetry 时间单调但因主动 kill 被脚本标为 suspect；无事件、无近停。最近旧起点前 `x≈-10,y≈-27` 距离约 `0.68`，最近 `x≈28,y≈-28` 距离约 `1.24`，都以 `normal` 和非近停速度通过。
- 判断：旧起点前卡点也未复现；车阵、起点前直道、下一轮左弯和上方复合弯 overlay 均未见栏杆/车辆误锁。

**R036 final 单文件冒烟**（2026-06-12，已完成，不跑完整场）：
- 配置：官方 `run_local.py --code-path submissions/final/team_controller.py --world complex --car-slot car_1`，不使用 debug 构建，AI 跑过旧起点前窗口后在 `t≈360.32s` 主动停止。
- 结果：telemetry 时间单调但因主动 kill 被脚本标为 suspect；无事件、无近停。最近旧 `x≈169,y≈111` 距离约 `0.72`，旧 `x≈-42,y≈124` 距离约 `1.98`，旧起点前 `x≈-10,y≈-27` / `x≈28,y≈-28` 距离约 `0.68` / `1.24`。
- 判断：实际上传的 final 单文件也复现 R035 的通过状态；它没有相机帧，不能替代 R035/R034 的视觉 overlay。

**回归门槛**：下一次验证优先让人跑当前候选，从视觉上确认第一个左转和后段复合弯是否稳定沿中间白色虚线；同时留意是否还有轻微擦栏。若肉眼仍见擦左，优先复盘 R029/R035 对应窗口的正常左弯舵角，而不是回退白线召回或继续堆 escape。case：`experiments/cases/R026_first_left_tight_radius/`（open）。
...
- Phase 2.2 候选的 AI 短测/长跑已覆盖第一个左弯、旧 `130→185s`、`280→330s`、旧 `x≈-42,y≈124`、旧起点前 `x≈-10,y≈-27` / `x≈28,y≈-28` 等风险窗，均未复现长爬行或内侧栏杆卡点；这仍是离线/AI 证据，不能替代人眼终判。
...
- R035 长跑里，车阵、起点前直道、下一轮左弯和上方复合弯 overlay 均显示白线候选在路面虚线附近，未见栏杆/车辆误锁；白线 offset 审计也显示多数窗口已贴近中线，但“视觉上稳定骑线”仍需人眼确认。
...
- R035 长跑通过旧起点前和下一轮早段的证据已按规则归档到 `experiments/figures/R035_long_loop_pass/`。这是 report figure，不是失败 case。
