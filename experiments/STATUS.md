...
最后更新：2026-06-13（R050 多车安全改进完成）。当前分支：`multi-car`。
...
1. **多车极端场景实跑验证（当前最优先）**：R050 已加入对手主动避让转向 + 丢线强制脱困（forward 替代 reverse），还需要 Webots 实跑验证三个极端场景：
   - 前车堵路：对手检测 + 降速 + 主动避让转向能否绕开
   - 被撞偏出：脱困机制能否恢复（pinned/boundary/low_speed/force_escape 链）
   - 卡栏杆：脱困链能否在 3-10s 内脱离
   - 测试方法见 `docs/multicar_extreme_tests.md`，一键启动 `bash scripts/webots_multicar_run.sh <world>`
2. **避让其他车（仍需增强）**：R050 新增了基于边界余量的对手避让转向（`opponent_avoid_steering`），但仍是反应式局部偏置而非规划层。对手检测也只覆盖前下方 ROI，不感知侧方/后方来车。后续可加：侧方碰撞预警、路径级绕行规划。
3. **个别中等弯偏宽/偏内**：根因是两个结构相似的弯被感知估出不同 `curve_risk`（弯在视野里发育多少、apex 遮挡、白线可见度）→ 不同速度 → 不同半径。这是**感知一致性**问题（见 notes.md R049）。继续提速也卡在这里：要更快只能让中等弯被正确识别，或接受更宽的弯。
4. **继续提速**：急弯已接近速度-半径物理上限，再快就更宽/撞。剩余空间在改善感知一致性 + 更好的过弯走线。
