...
**R037 final basic 回归**（2026-06-12，已完成，不跑完整场）：
- 配置：官方 `run_local.py --code-path submissions/final/team_controller.py --world basic --car-slot car_1`，AI 跑到 `t≈150.69s` 主动停止。
- 结果：telemetry 时间单调但因主动 kill 被脚本标为 suspect；无事件、无近停。真实速度 mean `5.12`，median `6.09`，p95 `6.10`。
- 判断：正式 final 单文件没有在 basic 前 150 秒引入明显回归，complex 红色环境单目兜底未污染 basic。
