...
### R005 — C003 蓝门桥接：局部修好但全局丢线 3 倍，已回退 (2026-06-11, basic)
- **构建**: 在 C002 基础上加"蓝色 checkpoint 门并入道路 mask"（gate-bridge）的调试构建。
- **配置**: world=basic, car_1, 单车, practice。
- **现象（用户肉眼）**: 蓝门前的无故拐弯**消失**（局部修复有效）；但第一个蓝门前会丢线，过弯仍贴栏杆、进第二段直道的弯**上了马路牙子**。
- **数据**: 控制日志 17558 帧 / 561.9s，**lost=0.54**（从开局就高，非后段退化）。离线在 frames_basic 复测：gate-bridge 让全局感知丢线 **0.227→0.660**，而蓝门窗口 t203-210 确实 0 lost。
- **根因**: 蓝门色相(H≈102)≈天空(H≈105)，门的 mask 把**天空/地平线蓝**也并进道路 → mask 饱和(>0.92) → 饱和惩罚 → 全局 lost 暴涨。加 `barrier_sat_max` 只从 0.660 降到 0.638，救不回来。
- **结论**: **回退 gate-bridge**（与 C001 同类错误：只验证了目标点、漏看全局回归）。保留 C002 的 grass-fix + pinned-escape。蓝门遮挡要换**不伪造 mask** 的思路（如 estimator 在远端 band 被遮挡时保持上一帧 heading/curvature）。
- **教训**: 任何感知改动必须在**整段录像 corpus** 上验证全局丢线，不能只看问题点。
